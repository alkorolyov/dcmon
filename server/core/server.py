...
try:
    from ..models import db_manager, MetricPoints, LogEntry
...
def create_app(config: ServerConfig) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        async def cleanup_loop():
            ...
            while True:
                try:
                    deleted_metrics = await loop.run_in_executor(None, MetricPoints.cleanup_old_data, config.metrics_days)
                    deleted_logs = await loop.run_in_executor(None, LogEntry.cleanup_old_logs, config.logs_days)
                    logger.debug("periodic cleanup: removed %s metric points (%s days), %s log entries (%s days)",
                                deleted_metrics, config.metrics_days, deleted_logs, config.logs_days)
                except Exception as e:
